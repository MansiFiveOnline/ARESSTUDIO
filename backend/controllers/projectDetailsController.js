...
//     const { project_id, sequence, ...otherFields } = req.body;

//     // Find the existing project detail
//     const existingProjectDetail = await projectDetailsModel.findById(_id);
//     if (!existingProjectDetail) {
//       return res.status(404).json({ message: "Project detail not found." });
//     }

//     // Check if sequence has changed
//     if (existingProjectDetail.sequence !== sequence) {
//       // Check for existing entries with the same project ID and new sequence
//       const conflictingEntry = await projectDetailsModel.findOne({
//         project_id,
//         sequence,
//       });
//       if (conflictingEntry) {
//         // Increment sequence of all subsequent entries
//         await projectDetailsModel.updateMany(
//           { project_id, sequence: { $gte: sequence } },
//           { $inc: { sequence: 1 } }
//         );
//       }
//     }
...
//     const updatedProjectDetail = await projectDetailsModel.findByIdAndUpdate(
//       _id,
//       { project_id, sequence, ...otherFields },
//       { new: true }
//     );
...
//   } catch (error) {
//     return res.status(500).json({
//       message: `Error in updating project detail due to ${error.message}`,
//     });
//   }
// };
